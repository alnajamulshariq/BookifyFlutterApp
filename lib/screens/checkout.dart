...
class _CheckoutState extends State<Checkout> {
  ...
  // Listen to cart items stream for updates
  Future<void> _loadCart() async {
    CartManager.getCartStream().listen((items) {
      setState(() {
        cartItems = items;
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      ...
      // Perform a batch delete to remove cart items
      ...
      // After a delay, navigate to HomeScreen
      Future.delayed(const Duration(seconds: 2), () {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => const HomeScreen()),
  ...
  // Helper method for displaying each summary row
  Row _summaryRow(String label, String value, {bool bold = false}) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
